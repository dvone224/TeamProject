	...
	public void startServer(String pythonFile) {
		try {
			if (socket == null || (socket != null && socket.isClosed())) {
				...
				// Python 파일 실행
				ProcessBuilder processBuilder = new ProcessBuilder("python", pythonFile + ".py");
...
//					System.out.println(line); // 여기서 에러 메시지도 출력됩니다.
//				}

				// 소켓 연결 대기
				socket = serverSocket.accept();
				this.reader = new InputStreamReader(socket.getInputStream(), java.nio.charset.StandardCharsets.UTF_8);
